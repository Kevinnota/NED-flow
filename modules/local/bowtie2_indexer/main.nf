process BOWTIE2_INDEXER {
    ...
    memory {
        ...
        def estimatedMem = Math.max(8, Math.min((double)(sizeGB * 20), (double)maxMemGB)).round(0)
    ...
    def estimatedTime = Math.max(3, Math.min((double)(sizeGB * 3), (double)maxTimeH)).round(0)
